...
export const BondedAreasChart: React.FC<BondedAreasChartProps> = ({
  ...
  const [rampupRates, setRampupRates] = useState<Record<string, number>>(() => {
    try {
      const saved = localStorage.getItem('byd_bonded_rampup_rates');
      if (saved) return { ...DEFAULT_BONDED_RAMPUP, ...JSON.parse(saved) };
    } catch {}
    return { ...DEFAULT_BONDED_RAMPUP };
  });
  const [workingDaysPerWeek, setWorkingDaysPerWeek] = useState<5 | 6>(() => {
    try {
      const saved = localStorage.getItem('byd_bonded_working_days');
      if (saved === '6' || saved === '5') return Number(saved) as 5 | 6;
    } catch {}
    return 5;
  });
  ...
  // Sync bonded ramp-up configuration in real-time across users
  useEffect(() => {
    const unsub = onSnapshot(doc(db, 'config', 'bondedRampup'), (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.data();
        if (data.rates) {
          setRampupRates(prev => ({ ...prev, ...data.rates }));
          try {
            localStorage.setItem('byd_bonded_rampup_rates', JSON.stringify({ ...DEFAULT_BONDED_RAMPUP, ...data.rates }));
          } catch {}
        }
        if (data.workingDays === 5 || data.workingDays === 6) {
          setWorkingDaysPerWeek(data.workingDays);
          try {
            localStorage.setItem('byd_bonded_working_days', String(data.workingDays));
          } catch {}
        }
      }
    }, (err) => {
      console.warn("Falha no listener de bondedRampup:", err);
    });
    return () => unsub();
  }, []);
  ...
  const handleRateChange = (key: string, val: number) => {
    ...
    setRampupRates(newRates);
    try {
      localStorage.setItem('byd_bonded_rampup_rates', JSON.stringify(newRates));
    } catch {}
    setDoc(doc(db, 'config', 'bondedRampup'), { rates: newRates }, { merge: true }).catch(err => {
      console.warn("Falha ao salvar taxa no Firestore:", err);
    });
  };

  // Reset to default averages (TECON: 100, TPC: 62, INTERMARITIMA: 59, CLIA: 10)
  const handleResetRates = () => {
    const defaultRates = { ...DEFAULT_BONDED_RAMPUP };
    setRampupRates(defaultRates);
    try {
      localStorage.setItem('byd_bonded_rampup_rates', JSON.stringify(defaultRates));
    } catch {}
    setDoc(doc(db, 'config', 'bondedRampup'), { rates: defaultRates }, { merge: true }).catch(err => {
      console.warn("Falha ao resetar taxas no Firestore:", err);
    });
  };

  // Persist working days change
  const handleWorkingDaysChange = (days: 5 | 6) => {
    setWorkingDaysPerWeek(days);
    try {
      localStorage.setItem('byd_bonded_working_days', String(days));
    } catch {}
    setDoc(doc(db, 'config', 'bondedRampup'), { workingDays: days }, { merge: true }).catch(err => {
      console.warn("Falha ao salvar dias úteis no Firestore:", err);
    });
  };
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 p-2.5 rounded-xl bg-gray-50 dark:bg-slate-800/60 border border-gray-200 dark:border-slate-700 text-xs">
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center bg-white dark:bg-slate-800 p-0.5 rounded-lg border border-gray-200 dark:border-slate-700 shadow-3xs">
                  <button
                    type="button"
                    onClick={() => handleWorkingDaysChange(5)}
                  ...
                  <button
                    type="button"
                    onClick={() => handleWorkingDaysChange(6)}
                    className={`px-2 py-1 rounded-md text-xs font-bold transition-all cursor-pointer ${
